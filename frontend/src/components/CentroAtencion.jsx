...
const filtrarTareas = (tarea) => {
  ...
  // 🔥 Función para tomar una tarea
  async function handleTomarTarea() {
  ...
  // 🔥 Función para liberar una tarea
  async function handleLiberarTarea() {
    if (!tareaSeleccionada) return;
  ...
  async function handleFinalizarTarea() {
    if (!tareaSeleccionada) return;
  ...
  // 🔥 Función para devolver una tarea
  async function handleDevolverTarea() {
    if (!tareaSeleccionada) return;
  ...
  async function handleCancelarTarea() {
    if (!tareaSeleccionada) return;
...
async function handleCorregirTarea() {
  if (!tareaSeleccionada) return;
  ...
  return (
    ...
    <div className="pagination">
        {[...Array(Math.ceil(tareas.length / tareasPorPagina))].map((_, index) => (
          <button
            key={index}
