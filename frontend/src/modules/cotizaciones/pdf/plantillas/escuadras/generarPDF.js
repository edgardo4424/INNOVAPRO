...
export default async function generarPDFEscuadras(doc, data) {
  ...
  let currentY = 50;
  console.log("generarPDFEscuadra", data);
  if (data.cotizacion?.tipo_servicio === "Venta") {
    // Si es una cotización de venta, usamos el cuerpo específico para venta
    currentY = await generarCuerpoEscuadrasVenta(doc, data, currentY); // Genera el cuerpo del PDF para escuadras en venta
    ...
    currentY = await generarCuerpoEscuadras(doc, data, currentY); // Genera el cuerpo del PDF para escuadras
    if (data.piezasAdicionales?.length > 0) {
      currentY = await renderPiezasAdicionales(doc, data, currentY);
    }
    currentY = await renderInstalacion(doc, data, currentY); // Renderiza la sección de instalación si existe
