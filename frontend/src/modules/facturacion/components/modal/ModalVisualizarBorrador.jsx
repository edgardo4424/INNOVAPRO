        ...
        const BuscarDocumento = async () => {
            try {
                ...
                const borradorMapeado = {
                    ...
                    cliente_razon_social: rawBorrador.cliente_Razon_Social.trim(), // Elimina espacios en blanco
                    cliente_direccion: rawBorrador.cliente_Direccion,
                    monto_oper_gravadas: rawBorrador.monto_Oper_Gravadas,
                    monto_oper_exoneradas: rawBorrador.monto_Oper_Exoneradas,
                    monto_igv: rawBorrador.monto_Igv,
                    total_impuestos: rawBorrador.total_Impuestos,
                    valor_venta: rawBorrador.valor_Venta,
                    sub_total: rawBorrador.sub_Total,
                    ...
                    detraccion_cta_banco: rawBorrador.detraccion_cta_banco,
                    detraccion_percent: rawBorrador.detraccion_percent,
                    detraccion_mount: rawBorrador.detraccion_mount,
                    descuento_cod_tipo: rawBorrador.descuento_cod_tipo,
                    descuento_monto_base: rawBorrador.descuento_monto_base,
                    descuento_factor: rawBorrador.descuento_factor,
                    descuento_monto: rawBorrador.descuento_monto,
                    detalle: rawBorrador.detalle ?? [],
                    forma_pago: rawBorrador.forma_pago ?? [],
                    legend: rawBorrador.legend ?? [],
                };

                setFactura(borradorMapeado);
            } catch (e) {
                console.error(e);
                ...
                <div className="relative w-full max-w-4xl p-6 bg-white shadow-2xl rounded-xl animate-scale-in overflow-y-auto max-h-[95vh]">
                    ...
                    <div className="mb-6 text-center border-b pb-4">
                        ...
                        <p className="text-lg text-gray-700">
                            Revisa los detalles de tu borrador antes de emitirlo como documento Oficial.
                        </p>
                    </div>
                                        ...
                                        {factura.detalle?.length ? (
                                            factura.detalle.map((item, index) => (
                                                <tr key={index} className="hover:bg-gray-50 border-b last:border-b-0">
                                                    ...
                                                    <td className="py-2 px-4 text-right text-sm">{formatNumber(item.monto_Valor_Unitario)}</td>
                                                    <td className="py-2 px-4 text-right text-sm">{formatNumber(item.monto_Precio_Unitario)}</td>
                                                    <td className="py-2 px-4 text-right text-sm">
                                                        {formatNumber(item.cantidad * (item.monto_Precio_Unitario ?? 0))}
                                                    </td>
                                                </tr>
                                            ))
                                        ) : (
                                    ...
                                    <ul className="list-disc list-inside text-sm text-gray-700">
                                        {factura.forma_pago.map((pago, index) => (
                                            <li key={index} className="mb-1">
                                                {pago.tipo} - {formatNumber(pago.monto)} {factura.tipo_moneda}
                                            </li>
                                        ))}
                                    </ul>
                                ) : (
                            ...
                            <div className="text-right p-4 border border-gray-200 rounded-md bg-white">
                                <p className="flex justify-between text-sm mb-1">
                                    ...
                                    <span className="text-gray-900">
                                        {factura.tipo_moneda} {formatNumber(factura.monto_oper_gravadas)}
                                    </span>
                                </p>
                                <p className="flex justify-between text-sm mb-1">
                                    <span className="font-semibold text-gray-700">Op. Exoneradas:</span>
                                    <span className="text-gray-900">
                                        {factura.tipo_moneda} {formatNumber(factura.monto_oper_exoneradas)}
                                    </span>
                                </p>
                                <p className="flex justify-between text-sm mb-1">
                                    <span className="font-semibold text-gray-700">IGV (18%):</span>
                                    <span className="text-gray-900">
                                        {factura.tipo_moneda} {formatNumber(factura.total_impuestos)}
                                    </span>
                                </p>
                                <p className="flex justify-between text-lg font-bold mt-4 pt-4 border-t-2 border-gray-300 text-blue-800">
                                    <span>TOTAL:</span>
                                    <span>
                                        {factura.tipo_moneda} {formatNumber(factura.sub_total)}
                                    </span>
                                </p>
                            </div>
                        </div>
