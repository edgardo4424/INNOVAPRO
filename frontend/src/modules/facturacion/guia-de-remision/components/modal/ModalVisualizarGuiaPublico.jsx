                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 bg-opacity-10 animate-fade-in">
                    <div className="relative w-full max-w-4xl p-6 bg-white shadow-2xl rounded-xl animate-scale-in overflow-y-auto max-h-[95vh]">
                        ...
                        <div className="border border-gray-300 p-6 rounded-lg bg-gray-50 text-gray-800">
                            ...
                            <div className="mb-6">
                                <h3 className="font-bold text-md mb-2 text-gray-600 border-b pb-1">DATOS DEL CONDUCTOR:</h3>
                                {guiaTransporte.chofer && guiaTransporte.chofer.length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {guiaTransporte.chofer.map((chofer, idx) => (
                                            <div key={idx} className="p-3 border border-gray-200 rounded-md bg-white text-sm">
                                                ...
                                                <p><strong>Apellidos:</strong> {chofer.apellidos || 'N/A'}</p>
                                                <p><strong>Nro. MTC:</strong> {chofer.nro_mtc || 'N/A'}</p> {/* Nuevo campo para guiaPublica */}
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-sm text-gray-500">No se han registrado datos de conductor.</p>
                                )}
                            </div>
                        ...
                        {/* No se incluye el componente EnviarFactura ya que es para facturas, no guías */}
                    </div>
                </div>
            )}
        </>
