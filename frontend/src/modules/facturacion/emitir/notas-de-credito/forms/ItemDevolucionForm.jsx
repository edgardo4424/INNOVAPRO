...
const ItemDevolucionForm = ({ closeModal }) => {
    ...
    const handleInput = (e) => {
        ...
        // Validar que solo se ingresen números válidos
        if ((name === "cantidad" || name === "monto_Valor_Unitario") && !/^\d*\.?\d*$/.test(value)) return;

        // Validación de límites basados en el item original
        ...
        const cantidad = name === "cantidad" ? parseFloat(value || 0) : parseFloat(itemDevolucionForm.cantidad || 0);
    ...
    const handleSave = () => {
        // Agregar el item a la lista de devolución
        const newItem = {
            ...itemDevolucionForm,
            id: itemActual.id || Date.now(), // Usar el ID del item original o generar uno nuevo
        };

        // Actualizar la lista de detalle
        setNotaCreditoDebito(prev => ({
            ...prev,
            detalle: prev.detalle.some(item => item.id === newItem.id)
                ? prev.detalle.map(item => item.id === newItem.id ? newItem : item)
                : [...prev.detalle, newItem]
        }));
            ...
            >
                ...
                <div className='flex flex-col gap-1 col-span-full md:col-span-2 lg:col-span-1'>
                    ...
                    <Input
                        ...
                        step="0.01"
                        min="0"
                        max={itemActual?.cantidad || undefined}
                    />
                    {itemActual.id > 0 && (
                        <span className='text-xs text-gray-500'>
                            Cantidad Límite: {itemActual.cantidad}
                        </span>
                    )}
                </div>
                ...
                <div className='flex flex-col gap-1 col-span-full md:col-span-2 lg:col-span-1'>
                    ...
                    <Input
                        ...
                        className="border-1 border-gray-400"
                        min="0"
                        max={itemActual?.monto_Valor_Unitario || undefined}
                    />
                    {itemActual.id > 0 && (
                        <span className='text-xs text-gray-500'>
                            Monto Unitario Límite: {itemActual.monto_Valor_Unitario}
                        </span>
                    )}
                </div>
