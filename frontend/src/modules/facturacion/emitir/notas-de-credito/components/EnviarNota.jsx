    ...
    const handleEmitirNotaClick = async () => {
        ...
        try {
            ...
            if (result.success) {
                // Caso de éxito
                setStatus("success");
                setDisplayMessage(result.message || "Nota emitida con éxito.");
            } else {
                // Caso de error lógico desde la API (ej. validaciones de SUNAT)
                setStatus("error");
                setDisplayMessage(result.message || "Error al emitir la nota.");
                setDetailedMessage(result.detailed_message || "");
            }
        } catch (err) {
            // Manejo de errores de red o del sistema
            setStatus("error");
            setDisplayMessage(err.message || "Error inesperado al emitir la nota.");
