...
import {
  Arraydetracciones,
  ArrayFormasDePago,
} from "../../utils/codDetraccion";
import { Switch } from "@/components/ui/switch";
import { valorIncialDetracion } from "../../utils/valoresInicial";
...
const DatosDeDetraccion = () => {
  ...
  // ** montos base para el cálculo
  let montoBase = monto_Imp_Venta - monto_Oper_Exoneradas;
  let montoBaseDolares =
    (monto_Imp_Venta - monto_Oper_Exoneradas) * precioDolarActual;
  const resetearMontoDetraccion = () => {
    if (detraccion.detraccion_percent) {
      let nuevoMontoCalculado;
      if (factura.tipo_Moneda === "PEN") {
        nuevoMontoCalculado = Number(
    ...
    <div className="overflow-y-auto p-4 sm:p-6 lg:p-8">
      <div>
        <div className="flex items-center gap-x-4">
          <h1 className="text-lg font-bold md:text-2xl">Detracción</h1>
          <Switch
            checked={detraccionActivado}
            onCheckedChange={handleSwitchChange}
            id="detraccion-switch"
            aria-label="Activar retención"
            className={detraccionActivado ? "!bg-blue-500" : "bg-gray-200"}
          />
        </div>
        {factura.tipo_Operacion == "1001" && detraccionActivado && (
