...
export default function CompGestionTrabajadoresonent() {
   ...
   const fetchTrabajadores = async () => {
      try {
         ...
         setError(
            "Error al cargar los datos de trabajadores. Por favor, intenta nuevamente."
         );
      } finally {
         setLoading(false);
      }
   ...
   const filteredEmployees = useMemo(() => {
      return trabajadores.filter((employee) => {
         ...
         const matchesArea =
            areaFilter === "all" || employee.cargo.area.nombre === areaFilter;
   ...
   const stats = useMemo(() => {
      ...
      const totalSalary = filteredEmployees.reduce(
         (sum, emp) => sum + emp.sueldo_base,
         0
      );
      const avgSalary = totalEmployees > 0 ? totalSalary / totalEmployees : 0;
      const afpCount = filteredEmployees.filter(
         (emp) => emp.sistema_pension === "AFP"
      ).length;
      ...
      <div className="space-y-6 p-6">
         ...
         <div className="flex justify-between items-center">
            <div>
               ...
               <p className="text-muted-foreground">
                  Administra y visualiza la información de tu personal
               </p>
            </div>
         </div>
         ...
         <Card>
            <CardHeader>
               ...
               <CardDescription>
                  Filtra y busca empleados por diferentes criterios
               </CardDescription>
            </CardHeader>
            <CardContent>
               <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <Select
                     value={pensionFilter}
                     onValueChange={setPensionFilter}
                  >
                     <SelectTrigger className="w-full md:w-[200px]">
         ...
         <Card>
            <CardHeader>
               ...
               <CardDescription>
                  Mostrando {filteredEmployees.length} de {trabajadores.length}{" "}
                  empleados
               </CardDescription>
            </CardHeader>
            <CardContent>
                           ...
                           return (
                              <TableRow key={employee.id}>
                                 ...
                                 <TableCell className="font-medium">
                                    {formatCurrency(employee.sueldo_base)}
                                 </TableCell>

                                 <TableCell>
                                    <Badge
                                       variant={
                                          employee.sistema_pension === "AFP"
                                             ? "default"
                                             : "secondary"
                                       }
                                    >
