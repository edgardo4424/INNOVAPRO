const {
  ...
  obtenerContactos,
  obtenerUnContacto,
} = require("./contacto.usecase");
const { resSuccess } = require("../../utils/response");
const { contactoCreateSchema } = require("./contacto.schema");

exports.crear = async (req, res, next) => {
  try {
    // ✅ Validar el cuerpo del request

    const data = contactoCreateSchema(req.body);

    const contacto = await crearContactoConRelaciones(data);
    return resSuccess({
      res,
