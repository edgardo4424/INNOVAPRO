module.exports = async (
  ...
  if (!condicion) {
    return {
      codigo: 404,
      respuesta: {
        mensaje: "No se encontró la condición para el contrato asociado",
      },
    };
  }

  const condicionesRawCompleto =
    condicion.condiciones.split("CONDICIONES AUTORIZADAS:")[1] || "";

  // Eliminamos todo lo que venga después de OBSERVACIÓN:
  const condicionesLimpias =
    condicionesRawCompleto.split("OBSERVACIÓN:")[0] || "";
  ...
  await condicionRepository.actualizarCondicionesCumplidas(
    contratoId,
    condicionesCumplidas,
    todasCumplidas
  );
  ...
  if (todasCumplidas) {
    await contratoRepository.actualizarEstadoCondiciones(
      contratoId,
      "Condiciones Cumplidas"
    );
  }

  // Verificar si existe un pase de pedido automático y actualizar su estado

  const pasePedido = await pasePedidoRepository.obtenerPasePedidoPorContratoId(
    contratoId
  );
  if (pasePedido) {
