...
module.exports = async (
  ...
  const fecha_fin =
    contratos_sin_interrupcion[contratos_sin_interrupcion.length - 1].fecha_fin;

  //Obtener vacaciones aprobadas entre las fechas de los contratos sin interrupcion
  ...
  //Conteo de vacaciones tomadas y vendidos anteriomenete, solo tomando en cuenta contratos sin iterupcions
  ...
  for (const a of asistencias_ordenadas) {
    ...
    const verificacionAsistencia = await asistenciaRepository.verificarAsistenciaPorTrabajadorFecha(
        trabajador.id,
        a.fecha,
        transaction
      );
    if (trabajador.id == 7) {
      console.log("Verificacion Asistencia: ", verificacionAsistencia);
    }
    if (verificacionAsistencia) {
      const estado_asistencia_values = ["vacacion-gozada", "vacacion-vendida"];
      if (
        estado_asistencia_values.includes(
          verificacionAsistencia.estado_asistencia
        )
      ) {
        throw new Error(`El dia ${formatearFecha(a.fecha)} ya cuenta con vacaciones creadas.`);
      }
      const actualizacionAsistencia =
        await asistenciaRepository.actualizarEstadoAsistenciaPorId(
          verificacionAsistencia.id,
          payload_asistencia.estado_asistencia,
          transaction
        );

      asistencias_creadas.push(actualizacionAsistencia);
    } else {
      const response =
        await asistenciaRepository.crearAsistenciaDeTipoVacaciones(
          payload_asistencia,
          transaction
        );
  ...
  for (const a of asistencias_ordenadas) {
    ...
    const response_v_a = await vacacionesRepository.crearVacacionesXasitencias(
