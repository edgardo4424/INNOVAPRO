...
} = require("./calcularMesesComputablesSemestre");
const {
  calcularResumenGratificaciones,
} = require("./calcularResumenGratificaciones");
const {
  mapearParaReporteGratificaciones,
} = require("./mapearParaReporteGratificaciones");
const {
  obtenerFechasInicioFinParaCalculo,
} = require("./obtenerFechasInicioFinParaCalculo");
...
const {
  mapearInfoDetalleGratificacion,
} = require("./mapearInfoDetalleGratificacion");
const {
...
async function calcularComponentesGratificaciones(
  contratos,
  periodo,
  anio,
  dataMantenimiento
          ...
          porRegimen.map(async (p) => {
            ...
            const valor_hora_extra =
              p.sueldo_base >= umbral_monto_hora_extra
                ? dataMantenimiento.MONTO_POR_HORA_EXTRA_MAYOR
                : dataMantenimiento.MONTO_POR_HORA_EXTRA_MENOR;
 ...
 */
            ...
            const bonifPct =
              dataMantenimiento.PORCENTAJE_BONIFICACION_ESSALUD / 100;
            ...
            if (!trabajador.domiciliado) {
              renta5ta = +(
                gratiNeta *
                (dataMantenimiento.PORCENTAJE_DESCUENTO_5TA_CATEGORIA_NO_DOMICILIADO /
                  100)
              ).toFixed(2);
            }
            ...
            const contratosQueCumplen = contratos.filter((c) => {
              ...
              const fin = c.fecha_terminacion_anticipada
                ? new Date(c.fecha_terminacion_anticipada)
                : c.fecha_fin
              ...
              return (
                inicio <= new Date(fechaFinPeriodo) &&
                fin >= new Date(fechaInicioPeriodo)
              );
            });
            ...
            const banco =
              contratosQueCumplen[contratosQueCumplen.length - 1]?.banco || "";
            const numeroCuenta =
              contratosQueCumplen[contratosQueCumplen.length - 1]
                ?.numero_cuenta || "";
