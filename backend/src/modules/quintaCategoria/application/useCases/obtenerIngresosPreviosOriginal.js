...
class ObtenerIngresosPrevios {
  constructor({ quintaRepo, bonoRepo, gratiRepo, asistenciaRepo } = {}) {
    ...
    async execute({ 
        ...
        if (filialId) {
            const gratiJulio = await this.gratiRepo.obtenerGratificacionPorTrabajador("JULIO", anio, filialId, trabajadorId) ;
            const gratiDiciembre = await this.gratiRepo.obtenerGratificacionPorTrabajador("DICIEMBRE", anio, filialId, trabajadorId);
            
            console.log("Grati de julio: ", gratiJulio?.total_pagar || 0);
            console.log("Grati de diciembre: ", gratiDiciembre?.total_pagar || 0);
           ...
           /*  const trabajadorGratiJulio = gratiJulio?.planilla?.trabajadores?.find(
            ...
            if (Number(mes) <= 6) {
                gratiJulioProj = gratiJulio?.total_pagar || 0;
                gratiDiciembreProj = gratiDiciembre?.total_pagar || 0;
                gratificaciones_total = 0;
            } else if (Number(mes) >= 7 && Number(mes) <= 11) {
                gratiJulioTrabajador = Number(gratiJulio?.total_pagar) || 0;
                gratiDiciembreProj = Number(gratiDiciembre?.total_pagar) || 0;
                gratificaciones_total = Number(gratiJulioTrabajador);
            } else {
                gratiJulioTrabajador = Number(gratiJulio?.total_pagar) || 0;
                gratiDiciembreTrabajador = Number(gratiDiciembre?.total_pagar) || 0;
        ...
        // Lee uit y deduccion fija y valor de hora extra
        const parametros = await getParametrosTributarios();
        const { valorHoraExtra } = confirmarParametrosTributarios(parametros)

        console.log("VALOR DE HORA EXTRA: ", valorHoraExtra)
        ...
        console.log("Las horas extras acumuladas del mes actual comenzando desde: ", ymd(primerDiaDelMesActual),
            ...
            " y su pago por esas horas son: S/", horasExtrasDelMes * valorHoraExtra
        )
