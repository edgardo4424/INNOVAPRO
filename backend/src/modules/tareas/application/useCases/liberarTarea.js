...
module.exports = async (idTarea, idUsuario, tareaRepository) => {
  ...
  const tarea = await tareaRepository.liberarTarea(idTarea, idUsuario);

  if (!tarea) {
    return {
      codigo: 403,
      respuesta: { mensaje: "No puedes liberar esta tarea" },
    };
  }
  ...
  const notificacionParaElTecnico = {
    usuarioId: idUsuario,
    mensaje: `Has liberado la tarea #${tarea.id}`,
    tipo: "info",
  };
  ...
  const notificacionParaElCreador = {
    ...
    mensaje: `El técnico ${tarea.tecnico_asignado.trabajador?.nombres} ha liberado la tarea #${tarea.id}.`,
    tipo: "info",
  };
  ...
  return {
    codigo: 200,
    respuesta: {
      mensaje: "Tarea liberada con éxito",
      tarea: tarea,
    },
  };
