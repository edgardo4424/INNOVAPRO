...
CREATE TABLE `gratificaciones` (
  `id` int NOT NULL AUTO_INCREMENT,
  `trabajador_id` int NOT NULL,
  `tipo_contrato` enum('PLANILLA','HONORARIO') NOT NULL DEFAULT 'PLANILLA',
  `periodo` varchar(7) NOT NULL,
  `fecha_calculo` date NOT NULL,
  `regimen` enum('GENERAL','MYPE','MICRO') NOT NULL,
  `factor_regimen` decimal(4,2) NOT NULL COMMENT '1.00 general, 0.50 mype, 0.00 micro',
  `sueldo_base` decimal(10,2) NOT NULL,
  `asignacion_familiar` decimal(10,2) NOT NULL DEFAULT '0.00',
  `promedio_horas_extras` decimal(10,2) NOT NULL DEFAULT '0.00',
  `promedio_bono_obra` decimal(10,2) NOT NULL DEFAULT '0.00',
  `remuneracion_computable` decimal(10,2) NOT NULL COMMENT 'suma de los 4 componentes',
  `meses_computables` int NOT NULL,
  `gratificacion_bruta` decimal(10,2) NOT NULL,
  `faltas_dias` int NOT NULL DEFAULT '0',
  `faltas_monto` decimal(10,2) NOT NULL DEFAULT '0.00',
  `no_computable` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT 'solo informativo, no entra a RC',
  `gratificacion_neta` decimal(10,2) NOT NULL COMMENT 'grati bruta - faltas',
  `sistema_salud` enum('ESSALUD','EPS') NOT NULL DEFAULT 'ESSALUD',
  `bonificacion_extraordinaria` decimal(10,2) NOT NULL DEFAULT '0.00',
  `renta_5ta` decimal(10,2) NOT NULL DEFAULT '0.00',
  `adelantos` decimal(10,2) NOT NULL DEFAULT '0.00',
  `total_pagar` decimal(10,2) NOT NULL,
  `usuario_calculo_id` int NOT NULL,
  `locked_at` datetime DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_trabajador_periodo` (`trabajador_id`,`periodo`),
  KEY `fk_grati_usuario` (`usuario_calculo_id`),
  CONSTRAINT `fk_grati_trabajador` FOREIGN KEY (`trabajador_id`) REFERENCES `trabajadores` (`id`),
  CONSTRAINT `fk_grati_usuario` FOREIGN KEY (`usuario_calculo_id`) REFERENCES `usuarios` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
